# Find diacritics with disabled auto alignment (fixed)

A GlyphsApp utility for find diacritics with disabled auto alignment.

## Requirements

- Glyphs App 3.x
- macOS
- Vanilla, included with current Glyphs installations

## Usage

1. Open a font in Glyphs.
2. Select the relevant glyphs, layers, nodes, or master when the tool requires a selection.
3. Run **Find diacritics with disabled auto alignment (fixed)** from the Scripts menu.
4. Review the options and the Macro Panel output before continuing with production files.

## License

Apache 2.0

...
class TransformationTool(object):
    ...
    def __init__(self):
        ...
        scale_metrics = Glyphs.defaults.get(self.prefix + "scale_metrics", True)
        ...
        # Checkbox para Scale metrics
        self.w.scaleMetricsCheckbox = vanilla.CheckBox(
            (15, 245, 150, 20),
            "Scale metrics",
            sizeStyle='small'
        )
        self.w.scaleMetricsCheckbox.set(scale_metrics)
    ...
    def save_values(self):
        ...
        Glyphs.defaults[self.prefix + "scale_metrics"] = self.w.scaleMetricsCheckbox.get()
    ...
    def create_master_controls(self):
        ...
        # Posición Y donde comenzarán los controles (ahora después del nuevo checkbox)
        start_y = 270
        y_pos = start_y
    ...
    def transform_component(self, component, values):
        """Aplica las transformaciones a un component mediante su matriz de transformación"""
        # Obtenir la matriu actual del component (si en té)
        transform = component.transform
        
        # Si no hi ha matriu, crear una d'identitat
        if not transform:
        ...
        # Aplicar les nostres transformacions:
        # 1. Escalat (multiplicar a i d)
        a *= values['scale'][0]
        d *= values['scale'][1]
        ...
        if values['slant'] != 0:
            slant_rad = math.radians(values['slant'])
            c = c + (d * math.tan(slant_rad))
        
        # 3. Traslació (sumar a tx i ty)
        tx += values['translate'][0]
        ty += values['translate'][1]
        
        # Actualitzar la matriu del component
        component.transform = (a, b, c, d, tx, ty)
    
    def apply_transform_to_layer(self, layer, values, scale_metrics):
        """Aplica les transformacions a una capa (paths, components, anchors i mètriques)"""
        ...
        # Transformar components (NOVA PART)
        if hasattr(layer, 'components') and layer.components:
            for component in layer.components:
                self.transform_component(component, values)
        ...
        # Escalar métricas si está activado
        if scale_metrics:
            scale_x = values['scale'][0]
            if hasattr(layer, 'LSB'):
                layer.LSB = layer.LSB * scale_x
            if hasattr(layer, 'RSB'):
                layer.RSB = layer.RSB * scale_x
            if hasattr(layer, 'width'):
                layer.width = layer.width * scale_x
    ...
    def apply(self, sender):
        ...
        # Obtener flag de scale metrics
        scale_metrics = self.w.scaleMetricsCheckbox.get()
            ...
            for layer in selected_layers:
                ...
                if use_selected_masters:
                        ...
                        if l.isMasterLayer or l.isSpecialLayer:
                            if hasattr(l, 'associatedMasterId'):
                                ...
                                if master_id in selected_master_ids:
                                    self.apply_transform_to_layer(l, values, scale_metrics)
                    ...
                    if scope == 0:  # Current Master
                        self.apply_transform_to_layer(layer, values, scale_metrics)
                        transformed_layers += 1
                        glyph_transformed = True
                    else:  # All Masters
                        for l in glyph.layers:

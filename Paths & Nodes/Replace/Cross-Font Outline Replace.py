...
class ReplaceOutlines(object):
    ...
    def __init__(self):
        ...
        self.w.putInBackgroundCheck = vanilla.CheckBox(
            (15, 120, -15, 20),
            "Put in background",
            value=False
        )
    ...
    def radioCallback(self, sender):
        ...
        if sender == self.w.currentMasterRadio:
            self.w.currentMasterRadio.set(True)
            self.w.allMastersRadio.set(False)
            self.applyToAllMasters = False
        elif sender == self.w.allMastersRadio:
            self.w.currentMasterRadio.set(False)
            self.w.allMastersRadio.set(True)
            self.applyToAllMasters = True
    ...
    def replaceGlyphs(self, sender):
        ...
        putInBackground = self.w.putInBackgroundCheck.get()
        ...
        actionPast = "Placed in background" if putInBackground else "Replaced"
        actionAdverb = "placed in background" if putInBackground else "replaced"
        ...
        if failedGlyphs:
            ...
            Glyphs.showNotification(
                ...
                f"{actionPast} {successCount} glyphs in {masterMode}. Failed: {failedList}"
            )
        else:
            Glyphs.showNotification(
                "Replace Outlines - Complete",
                f"Successfully {actionAdverb} {successCount} glyphs in {masterMode}."
            )
        
    # -----------------------------------------------------
    
    def replaceLayer(self, targetGlyph, sourceGlyph, targetMasterId, sourceMasterId, putInBackground=False):
        """Replace layer content or copy source outlines into the background"""
        ...
        if putInBackground:
            self.copySourceToBackground(targetLayer, sourceLayer)
            return
    ...
    def copySourceToBackground(self, targetLayer, sourceLayer):
        """Put source outlines/components in the target layer background."""

        backgroundLayer = targetLayer.background

        # Clear existing background shapes, then place the source outline there.
        backgroundLayer.shapes = []
        for shape in sourceLayer.shapes:
            newShape = shape.copy()
            backgroundLayer.shapes.append(newShape)

        # Copy components to background
        backgroundLayer.components = []
        for component in sourceLayer.components:
            newComponent = component.copy()
            backgroundLayer.components.append(newComponent)

        # Keep the background aligned with the source glyph metrics without touching the foreground.
        backgroundLayer.width = sourceLayer.width
        backgroundLayer.LSB = sourceLayer.LSB
        backgroundLayer.RSB = sourceLayer.RSB

        if sourceLayer.backgroundImage:
            backgroundLayer.backgroundImage = sourceLayer.backgroundImage.copy()
        else:
            backgroundLayer.backgroundImage = None

...
class MoveElementsClean(object):
    ...
    def getNodesToMove(self, path, selectedNodeIndexes):
        """
        Obtiene todos los nodos a mover incluyendo handles conectados.
        Usa las relaciones nativas de Glyphs (nextNode, prevNode) en lugar de índices.
        """
        nodesToMove = set()
        allNodes = path.nodes
        
        # Primero, obtener los nodos seleccionados
        selectedNodes = []
        for i in selectedNodeIndexes:
            if i < len(allNodes):
                selectedNodes.append(allNodes[i])
        
        # Para cada nodo seleccionado, añadir sus handles conectados
        for node in selectedNodes:
            nodesToMove.add(node)
            
            # Usar las propiedades nativas para encontrar nodos conectados
            # En Glyphs, los handles off-curve están conectados a nodos on-curve
            # mediante nextNode y prevNode
            
            # Explorar hacia adelante (nextNode) hasta encontrar un nodo on-curve
            # Esto captura handles que están después del nodo seleccionado
            nextNode = node.nextNode
            while nextNode is not None and nextNode.type == OFFCURVE:
                nodesToMove.add(nextNode)
                nextNode = nextNode.nextNode
            
            # Explorar hacia atrás (prevNode) hasta encontrar un nodo on-curve
            # Esto captura handles que están antes del nodo seleccionado
            prevNode = node.prevNode
            while prevNode is not None and prevNode.type == OFFCURVE:
                nodesToMove.add(prevNode)
                prevNode = prevNode.prevNode
        
        # También manejar el caso inverso: si se selecciona un handle,
        # asegurarse de que su nodo on-curve también se mueva
        handlesToProcess = [n for n in selectedNodes if n.type == OFFCURVE]
        for handle in handlesToProcess:
            # Buscar el nodo on-curve conectado a este handle
            # (el handle es off-curve, sus next/prev son on-curve o continuaciones)
            if handle.nextNode is not None and handle.nextNode.type != OFFCURVE:
                nodesToMove.add(handle.nextNode)
            if handle.prevNode is not None and handle.prevNode.type != OFFCURVE:
                nodesToMove.add(handle.prevNode)
    ...
    def move(self, xDir, yDir):
        ...
        for layer in font.selectedLayers:
            ...
            move_entire_glyph = not has_selection
                ...
                if mode in (0, 2):
                    ...
                    for pIndex, path in enumerate(layer.paths):
                        ...
                        for target in targetLayers:
                            ...
                            # Obtener nodos a mover usando las relaciones nativas
                            nodesToMove = self.getNodesToMove(tPath, selectedNodeIndexes)

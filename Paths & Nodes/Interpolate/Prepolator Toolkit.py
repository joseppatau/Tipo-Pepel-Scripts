...
class GlyphTools(object):
    ...
    def __init__(self):
        ...
        self.w = vanilla.FloatingWindow((250, 750), "Prepolator Toolkit")
        ...
        # Botones Previous y Next en la misma fila
        self.w.previousNodeButton = vanilla.Button(
            (15, 80, 100, 25),
            "Previous Node",
            callback=self.previousNode
        )

        self.w.nextNodeButton = vanilla.Button(
            (125, 80, 100, 25),
            "Next Node",
            callback=self.nextNode
        )

        # Espacio de 20 píxeles
        self.w.spacer = vanilla.TextBox(
            (15, 115, -15, 20),
            ""
        )
        ...
        # Separator
        self.w.separator = vanilla.HorizontalLine((15, 415, -15, 1))
    ...
    def getAllOnCurveNodes(self, layer):
        """Retorna una llista de tots els nodes on-curve de la capa en ordre"""
        nodes = []
    ...
    def previousNode(self, sender):
        """Mou el primer node a la posició anterior (només al master actiu)"""
        font = Glyphs.font
        ...
        # Obtenir tots els nodes on-curve
        all_nodes = self.getAllOnCurveNodes(layer)
        
        if len(all_nodes) < 2:
            print("No hi ha suficients nodes per moure.")
            return
        
        # El primer node actual és l'índex 0
        current_first_node = all_nodes[0]
        
        # Trobar la posició actual del primer node a la llista completa
        try:
            current_index = all_nodes.index(current_first_node)
        except ValueError:
            print("Error: No s'ha trobat el node.")
            return
        
        # Calcular índex anterior (amb wrap-around)
        prev_index = (current_index - 1) % len(all_nodes)
        
        # L'objectiu és el node que volem que sigui el primer
        target_node = all_nodes[prev_index]
        
        # Fer que el node objectiu sigui el primer
        target_node.makeNodeFirst()
        
        print(f"Primer node mogut a la posició anterior. Node anterior: ({round(target_node.x,1)}, {round(target_node.y,1)})")

    def nextNode(self, sender):
        """Mou el primer node a la posició posterior (només al master actiu)"""
        font = Glyphs.font
        if not font.selectedLayers:
            print("Selecciona un glifo.")
            return
        ...
        # Calcular índex posterior (amb wrap-around)
        next_index = (current_index + 1) % len(all_nodes)
        
        # L'objectiu és el node que volem que sigui el primer
        target_node = all_nodes[next_index]
        
        # Fer que el node objectiu sigui el primer
        target_node.makeNodeFirst()
        
        print(f"Primer node mogut a la posició posterior. Node següent: ({round(target_node.x,1)}, {round(target_node.y,1)})")
